...
class RulesTestCase(unittest.TestCase):
  ...
  def testTurnCounter(self):
    ...
    self.rules.place_piece(self.board, p, 1)
    self.assertEqual(self.rules.turn_counter, 1)
    self.assertEqual(self.rules.active_player(), self.player2, 'the turn did not increment')
    self.assertEqual(self.rules.inactive_player(), self.player1, 'the inactive_player is not correct')
  ...
  def testMoves(self):
    ...
    player = self.rules.active_player()
    movable_pieces = self.rules.movable_pieces(self.board, player)
    move_set = [p.position for p in movable_pieces]
    self.assertEqual(move_set, [17], 'a piece which should be movable is not')
    moves_for_piece = self.rules.moves_for_piece(self.board, 17)
    self.assertEqual(moves_for_piece, {24})

    with self.assertRaises(ValueError):
      self.rules.move_piece(self.board, 19, 24)
    self.rules.move_piece(self.board, 17, 24)
  ...
  def testLongMoves(self):
    ...
    moves = self.rules.moves_for_piece(self.board, 1)
    self.assertEqual(moves, {8,7}, 'single long move not registering')
    moves = self.rules.moves_for_piece(self.board, 5)
    ...
    moves = self.rules.moves_for_piece(self.board, 6)
  ...
  def testMillCheck(self):
    ...
    self.assertTrue(self.rules.mill_check(self.board, 3, player), 'a mill was not detected')
  ...
  def testMillCheckAfterPlace(self):
    ...
    self.assertTrue(self.rules.place_piece(self.board, stones[2], 3))
    # it is still player1's turn to steal a piece
    self.assertEqual(self.rules.turn_counter, 0, 'the turn should not change after placing into a mill')
